...
export default function EditVendorForm({ id }) {
  ...
  useEffect(() => {
    if (!id) return;
    (async () => {
      const res = await API.getSingleVendor(id);
      if (res.success) {
        const v = res.message?.data ?? res.message;
        const photo = v.logo ?? v.avatar ?? v.photo ?? null;
        setFormData({
          name: v.name ?? "",
          email: v.email ?? "",
          phone: v.phone ?? "",
          website: v.website ?? "",
          address: v.address ?? "",
          company_id: v.companyId?._id ?? v.companyId?._id ?? "",
        });
        if (photo) {
          setExistingPhoto(photo);
          setPreview(`${process.env.NEXT_PUBLIC_ASSET_URL ?? ""}${photo}`);
        }
      } else {
  ...
  const handleSubmit = async (e) => {
    ...
    if (newPhoto) payload.append("logo", newPhoto);
    if (photoRemoved) payload.append("removeLogo", "true");

    const res = await API.updateVendor(id, payload);
    if (res.success) {
      toast.success(res.message?.message || "Vendor updated successfully!");
      router.push("/vendors");
    } else {
        ...
        <div className="flex flex-col gap-8">
          ...
          <div className="flex flex-col items-center">
            <div className="relative">
              ...
              <button
                type="button"
                onClick={preview ? clearPhoto : () => fileRef.current?.click()}
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-5">
            ...
            <div>
              <label className="block font-semibold mb-1.5 text-[12px] text-[#544b40]">
                Vendor Name <span className="text-[#c6212f]">*</span>
              </label>
              {fetching ? (
                <FieldSkeleton />
              ) : (
