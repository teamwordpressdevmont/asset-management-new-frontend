...
function StatusToggle({ active, loading, onToggle }) {
    return (
        <button type="button" onClick={onToggle} disabled={loading} title={active ? "Click to deactivate" : "Click to activate"}
            className={`cursor-pointer relative inline-flex h-5 w-9 items-center rounded-full transition-colors duration-200 disabled:opacity-50 disabled:cursor-not-allowed ${active ? "bg-[#C6212F]" : "bg-[#e5dfd3]"}`}>
...
export default function AssetsCategoryList() {
    ...
    const handleDelete = async (id) => {
        const result = await MySwal.fire({ title: "Delete Category?", text: "This action cannot be undone.", icon: "warning", showCancelButton: true, confirmButtonText: "Yes, delete", cancelButtonText: "Cancel" });
        if (!result.isConfirmed) return;
        setDeleting(id);
        const res = await API.deleteAssetCategory(id);
        if (res.success) { toast.success("Category deleted successfully."); fetchCategories(); }
    ...
    const handleToggleStatus = async (cat) => {
        ...
        const res = await API.updateAssetCategory(cat._id, { status: newStatus });
        if (res.success) { toast.success(`Category marked as ${newStatus}.`); }
        else { setCategories(prev => prev.map(c => c._id === cat._id ? { ...c, status: cat.status } : c)); toast.error(res.message || "Status update failed."); }
        setToggling(null);
    };
        ...
        <div className="space-y-5">
            <div className="flex items-center justify-between">
                <div>
                    ...
                    <h1 className="text-2xl font-semibold text-[#15100B]">Asset Categories</h1>
                    <p className="text-xs text-[#8e8576] mt-0.5">{loading ? "Loading..." : `${total} categor${total !== 1 ? "ies" : "y"} total`}</p>
                </div>
                <Link href="/assets-category/add" className="flex items-center gap-2 px-4 py-2.5 text-sm font-semibold text-white bg-[#C6212F] rounded-[10px] hover:bg-[#a81b27] transition-all duration-200 shadow-[0_4px_14px_rgba(198,33,47,0.3)] hover:shadow-[0_6px_20px_rgba(198,33,47,0.45)]">
            ...
            <div className="bg-white rounded-2xl overflow-hidden border border-[#f0ebe3]">
                <div className="flex items-center justify-between gap-3 px-5 py-4 border-b border-[#f0ebe3]">
                    <div className="flex items-center gap-2.5 bg-[#fbf8f2] border border-[#e5dfd3] rounded-[10px] px-3.5 py-0 shadow-sm w-72 focus-within:border-[#c6212f] focus-within:shadow-[0_0_0_3px_rgba(198,33,47,0.08)] transition-all">
                        ...
                        <input type="text" placeholder="Search categories..." value={searchInput} onChange={e => setSearchInput(e.target.value)} className="w-full py-2.5 bg-transparent text-sm text-[#15100B] placeholder-[#8e8576] border-none outline-none" />
                ...
                <div className="overflow-x-auto">
                    <table className="w-full text-sm table-fixed">
                        <colgroup>
                            <col style={{ width: "8%" }} />
                            <col style={{ width: isSuperAdmin ? "32.5%" : "65%" }} />
                            {isSuperAdmin && <col style={{ width: "32.5%" }} />}
                        ...
                        <thead>
                            <tr className="border-b border-[#f0ebe3] bg-[#fbf8f2]/80">
                                ...
                                <th className="text-left px-4 py-3 text-[11px] font-semibold text-[#8e8576] uppercase tracking-wider">Name</th>
                            ...
                            {loading ? (
                                [...Array(PER_PAGE)].map((_, i) => (
                                    <tr key={i} style={{ height: ROW_H }}>
                                        ...
                                        <td className="px-4"><div className="flex items-center gap-3"><div className="w-8 h-8 rounded-lg bg-black/6 animate-pulse shrink-0" /><Sk w="55%" /></div></td>
                                ...
                                <tr style={{ height: bodyH }}>
                                    <td colSpan={colCount} className="text-center align-middle">
                                        <div className="flex flex-col items-center gap-2">
                                            <svg width="36" height="36" viewBox="0 0 24 24" fill="none" stroke="#c6c0b5" strokeWidth="1.4" strokeLinecap="round" strokeLinejoin="round"><rect x="3" y="3" width="7" height="7" rx="1" /><rect x="14" y="3" width="7" height="7" rx="1" /><rect x="3" y="14" width="7" height="7" rx="1" /><rect x="14" y="14" width="7" height="7" rx="1" /></svg>
                                            <p className="text-sm text-[#8e8576]">{search ? `No results for "${search}"` : "No categories found"}</p>
                                ...
                                <>
                                    {categories.map((cat, idx) => (
                                        <tr key={cat._id} style={{ height: ROW_H }} className="hover:bg-[#fbf8f2]/60 transition-colors">
                                            <td className="px-4 text-[12px] font-medium text-[#8e8576]">{from + idx}</td>
                                            <td className="px-4">
                                                <div className="flex items-center gap-3 min-w-0">
                                                    <span className="font-medium text-[#15100B] truncate text-[13px]">{cat.name}</span>
                                                </div>
                                            </td>
                                            {isSuperAdmin && (
                                            <td className="px-4 text-[13px] text-[#544b40] truncate">
                                                {cat.companyId?.name || <span className="text-[#c6c0b5]">—</span>}
                                            </td>
                                            )}
                                            <td className="px-4 text-center">
                                                <StatusToggle active={cat.status === "active"} loading={toggling === cat._id} onToggle={() => handleToggleStatus(cat)} />
                                            </td>
                                            <td className="px-4">
                                                <div className="flex items-center justify-end gap-1">
                                                    <Link href={`/assets-category/${cat._id}/edit`} className="p-1.5 rounded-lg text-[#8e8576] hover:text-[#c6212f] hover:bg-[#c6212f]/8 transition-colors" title="Edit">
                                                        <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round"><path d="M11 4H4a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7" /><path d="M18.5 2.5a2.121 2.121 0 0 1 3 3L12 15l-4 1 1-4 9.5-9.5z" /></svg>
                                                    </Link>
                                                    <button onClick={() => handleDelete(cat._id)} disabled={deleting === cat._id} className="p-1.5 rounded-lg text-[#8e8576] hover:text-red-500 hover:bg-red-50 transition-colors disabled:opacity-40" title="Delete">
                                                        {deleting === cat._id
                                    ...
                                    {categories.length < PER_PAGE && [...Array(PER_PAGE - categories.length)].map((_, i) => (
