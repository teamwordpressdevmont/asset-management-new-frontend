...
function RoleBadge({ role }) {
    ...
    return <span className={`inline-flex items-center text-[11px] font-semibold px-2 py-0.5 rounded-full ${b.cls}`}>{b.label}</span>;
}

function StatusToggle({ active, loading, onToggle, disabled }) {
    return (
        <button type="button" onClick={onToggle} disabled={loading || disabled} title={disabled ? "Cannot change own status" : (active ? "Click to deactivate" : "Click to activate")}
            ...
            <span className={`inline-block h-3.5 w-3.5 rounded-full bg-white shadow-sm transition-transform duration-200 ${active ? "translate-x-[18px]" : "translate-x-0.5"}`} />
        </button>
    );
}
...
export default function UserList() {
    ...
    const handleDelete = async (id) => {
        const result = await MySwal.fire({ title: "Delete User?", text: "This action cannot be undone.", icon: "warning", showCancelButton: true, confirmButtonText: "Yes, delete", cancelButtonText: "Cancel" });
        if (!result.isConfirmed) return;
        setDeleting(id);
        const res = await API.deleteUser(id);
        if (res.success) { toast.success("User deleted successfully."); fetchUsers(); }
    ...
    const handleToggleStatus = async (user) => {
        ...
        if (res.success) { toast.success(`User marked as ${newStatus}.`); }
        else { setUsers(prev => prev.map(u => u._id === user._id ? { ...u, status: user.status } : u)); toast.error(res.message || "Status update failed."); }
        setToggling(null);
    };
        ...
        <div className="space-y-5">
            <div className="flex items-center justify-between">
                <div>
                    ...
                    <p className="text-xs text-[#8e8576] mt-0.5">{loading ? "Loading..." : `${total} user${total !== 1 ? "s" : ""} total`}</p>
                </div>
                <Link href="/users/add" className="flex items-center gap-2 px-4 py-2.5 text-sm font-semibold text-white bg-[#C6212F] rounded-[10px] hover:bg-[#a81b27] transition-all duration-200 shadow-[0_4px_14px_rgba(198,33,47,0.3)] hover:shadow-[0_6px_20px_rgba(198,33,47,0.45)]">
            ...
            <div className="bg-white rounded-2xl overflow-hidden border border-[#f0ebe3]">
                <div className="flex items-center justify-between gap-3 px-5 py-4 border-b border-[#f0ebe3]">
                    <div className="flex items-center gap-2.5 bg-[#fbf8f2] border border-[#e5dfd3] rounded-[10px] px-3.5 py-0 shadow-sm w-72 focus-within:border-[#c6212f] focus-within:shadow-[0_0_0_3px_rgba(198,33,47,0.08)] transition-all">
                        ...
                        <input type="text" placeholder="Search users..." value={searchInput} onChange={e => setSearchInput(e.target.value)} className="w-full py-2.5 bg-transparent text-sm text-[#15100B] placeholder-[#8e8576] border-none outline-none" />
                ...
                <div className="overflow-x-auto">
                    <table className="w-full text-sm table-fixed">
                        <colgroup>
                            <col style={{ width: "4%" }} /><col style={{ width: isSuperAdmin ? "17%" : "21%" }} /><col style={{ width: isSuperAdmin ? "17%" : "21%" }} />
                        ...
                        <thead>
                            <tr className="border-b border-[#f0ebe3] bg-[#fbf8f2]/80">
                                ...
                                <th className="text-left px-4 py-3 text-[11px] font-semibold text-[#8e8576] uppercase tracking-wider">User</th>
                                ...
                                <th className="text-left px-4 py-3 text-[11px] font-semibold text-[#8e8576] uppercase tracking-wider">Role</th>
                            ...
                            {loading ? (
                                [...Array(PER_PAGE)].map((_, i) => (
                                    <tr key={i} style={{ height: ROW_H }}>
                                        ...
                                        <td className="px-4"><Sk w="60%" /></td>
                                        <td className="px-4"><Sk w="55%" /></td>
                                        {isSuperAdmin && <td className="px-4"><Sk w="65%" /></td>}
                                ...
                                <tr style={{ height: bodyH }}>
                                    <td colSpan={colCount} className="text-center align-middle">
                                        <div className="flex flex-col items-center gap-2">
                                            <svg width="36" height="36" viewBox="0 0 24 24" fill="none" stroke="#c6c0b5" strokeWidth="1.4" strokeLinecap="round" strokeLinejoin="round"><path d="M17 21v-2a4 4 0 0 0-4-4H5a4 4 0 0 0-4 4v2" /><circle cx="9" cy="7" r="4" /><path d="M23 21v-2a4 4 0 0 0-3-3.87M16 3.13a4 4 0 0 1 0 7.75" /></svg>
                                            <p className="text-sm text-[#8e8576]">{search ? `No results for "${search}"` : "No users found"}</p>
                                ...
                                <>
                                    {users.map((user, idx) => (
                                        <tr key={user._id} style={{ height: ROW_H }} className="hover:bg-[#fbf8f2]/60 transition-colors">
                                            <td className="px-4 text-[12px] font-medium text-[#8e8576]">{from + idx}</td>
                                            <td className="px-4"><div className="flex items-center gap-2.5 min-w-0"><Avatar name={user.name} photo={user.profileImage} /><span className="font-medium text-[#15100B] truncate text-[13px]">{user.name}</span></div></td>
                                            <td className="px-4 text-[13px] text-[#544b40] truncate">{user.email || <span className="text-[#c6c0b5]">—</span>}</td>
                                            <td className="px-4 text-[13px] text-[#544b40] truncate">{user.phone || <span className="text-[#c6c0b5]">—</span>}</td>
                                            <td className="px-4"><RoleBadge role={user.role?.name} /></td>
                                            {isSuperAdmin && <td className="px-4 text-[13px] text-[#544b40] truncate">{user.companyId?.name || <span className="text-[#c6c0b5]">—</span>}</td>}
                                            <td className="px-4 text-center"><StatusToggle active={user.status === "active"} loading={toggling === user._id} onToggle={() => handleToggleStatus(user)} disabled={user._id === currentUserId} /></td>
                                            <td className="px-4">
                                                {user._id !== currentUserId && (
                                                <div className="flex items-center justify-end gap-1">
                                                    <Link href={`/users/${user._id}/edit`} className="p-1.5 rounded-lg text-[#8e8576] hover:text-[#c6212f] hover:bg-[#c6212f]/8 transition-colors" title="Edit">
                                                        <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round"><path d="M11 4H4a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7" /><path d="M18.5 2.5a2.121 2.121 0 0 1 3 3L12 15l-4 1 1-4 9.5-9.5z" /></svg>
                                                    </Link>
                                                    <button onClick={() => handleDelete(user._id)} disabled={deleting === user._id} className="p-1.5 rounded-lg text-[#8e8576] hover:text-red-500 hover:bg-red-50 transition-colors disabled:opacity-40" title="Delete">
                                                        {deleting === user._id ? <svg className="animate-spin" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round"><path d="M21 12a9 9 0 1 1-6.219-8.56" /></svg>
                                    ...
                                    {users.length < PER_PAGE && [...Array(PER_PAGE - users.length)].map((_, i) => (
