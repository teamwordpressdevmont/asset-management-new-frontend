...
const initialFormData = { name: "", email: "", password: "", confirmPassword: "", phone: "", role: "user", company_id: "", active: true, photo: null };
const initialErrors = { name: "", email: "", password: "", confirmPassword: "", phone: "", company_id: "" };

function validate(data) {
    const e = { name: "", email: "", password: "", confirmPassword: "", phone: "", company_id: "" };
...
export default function AddUserForm() {
    ...
    const clearPhoto = () => { setFormData(prev => ({ ...prev, photo: null })); setPreview(null); };
    ...
    const handleSubmit = async (e) => {
        ...
        if (formData.photo) payload.append("profileImage", formData.photo);

        const res = await API.createUser(payload);
        if (res.success) {
            toast.success(res.message?.message || "User created successfully!");
            router.push("/users");
        } else {
                ...
                <div className="flex flex-col gap-8">
                    ...
                    <div className="flex items-center gap-5 mb-8">
                        ...
                        <div>
                            <div className="flex items-center gap-2 mb-1">
                                ...
                                <span className="text-xs text-[#c6212f] font-semibold tracking-widest uppercase">New User</span>
                            </div>
                            <h1 className="text-2xl font-semibold text-[#15100B]">Add User</h1>
                            <p className="text-xs text-[#8e8576] mt-0.5">Fill in the details to create a new user account</p>
                        </div>
                    </div>
                    ...
                    <div>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-5 mt-5">
                            {/* Password */}
                            <div>
                                <label className="block font-semibold mb-1.5 text-[12px] text-[#544b40]">Password <span className="text-[#c6212f]">*</span></label>
                            ...
                            {/* Confirm Password */}
                            <div>
                                <label className="block font-semibold mb-1.5 text-[12px] text-[#544b40]">Confirm Password <span className="text-[#c6212f]">*</span></label>
                                <div className={errors.confirmPassword ? wrapperError : wrapperNormal}>
                                    <span className="text-[#8e8576] grid place-items-center shrink-0">
                                        <svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.6" strokeLinecap="round" strokeLinejoin="round">
                                            <path d="M12 22s8-4 8-10V5l-8-3-8 3v7c0 6 8 10 8 10z" />
                                        </svg>
                                    </span>
                                    <input type={showConf ? "text" : "password"} name="confirmPassword" value={formData.confirmPassword} onChange={handleChange} placeholder="Re-enter password" className={inputBase} />
                                    <button type="button" onClick={() => setShowConf(p => !p)} className="text-[#8e8576] hover:text-[#c6212f] transition-colors shrink-0">
                                        {showConf ? (
                                            <svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round"><path d="M17.94 17.94A10.07 10.07 0 0 1 12 20c-7 0-11-8-11-8a18.45 18.45 0 0 1 5.06-5.94M9.9 4.24A9.12 9.12 0 0 1 12 4c7 0 11 8 11 8a18.5 18.5 0 0 1-2.16 3.19m-6.72-1.07a3 3 0 1 1-4.24-4.24" /><line x1="1" y1="1" x2="23" y2="23" /></svg>
                                        ) : (
                                            <svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round"><path d="M1 12s4-8 11-8 11 8 11 8-4 8-11 8-11-8-11-8z" /><circle cx="12" cy="12" r="3" /></svg>
                            ...
                            {/* Role */}
                            <div>
                                <label className="block font-semibold mb-1.5 text-[12px] text-[#544b40]">Role</label>
                                <CustomSelect
                                    name="role"
                                    value={formData.role}
                                    ...
                                    icon={<svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.6" strokeLinecap="round" strokeLinejoin="round"><path d="M17 21v-2a4 4 0 0 0-4-4H5a4 4 0 0 0-4 4v2" /><circle cx="9" cy="7" r="4" /><path d="M23 21v-2a4 4 0 0 0-3-3.87M16 3.13a4 4 0 0 1 0 7.75" /></svg>}
                                />
                            </div>

                            {/* Company — hidden for admin (auto-filled from their own company) */}
                            {!isAdmin && (
                            <div>
                ...
                <div className="flex justify-end mt-8 pt-6">
                    ...
                    <button type="submit" disabled={loading}
                        ...
                        {loading ? (
                            ...
                            <>Save User<svg className="ml-2" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="white" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M5 12H19M13 6L19 12L13 18" /></svg></>
                        )}
                    </button>
                </div>
            </form>
